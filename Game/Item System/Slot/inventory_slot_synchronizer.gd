...
## Dirty workaround
@export var _controlled_slot:InventorySlot

var controlled_slot:InventorySlot:
	set(new):
		if controlled_slot:
			if is_multiplayer_authority():
				controlled_slot.item_switched.disconnect(_on_server_item_switched)
		controlled_slot = new
		if controlled_slot:
			if is_multiplayer_authority():
				controlled_slot.item_switched.connect(_on_server_item_switched)

@export var synchronizer:MultiplayerSynchronizer
var sync_whitelist:Array[int] = []
	...
	set(new):
		...
		if !multiplayer.is_server():
			...
			if !synchronized_item_uid:
				controlled_slot.held_item = null
			var resource = Items.get_item_by_id(synchronized_item_uid)
			if !resource:
				return
			controlled_slot.held_item = ItemInstance.from_resource(resource)
...
@export var synchronized_item_properties:Dictionary = {}:
	set(new):
		...
		if is_multiplayer_authority():
			return
		if !controlled_slot.held_item:
			return
		controlled_slot.held_item.update_properties(synchronized_item_properties)
...
func _ready() -> void:
	controlled_slot = _controlled_slot
	if is_multiplayer_authority():
		_on_server_item_switched(controlled_slot.held_item)
